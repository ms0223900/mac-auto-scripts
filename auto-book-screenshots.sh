...
# 自動截圖腳本啟動器
# 用法: ./auto-book-screenshots.sh [--next-page "按鍵名稱"] [--wake-app]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --wake-app)
            ...
            ;;
        -h|--help)
            echo "用法: $0 [--next-page \"按鍵名稱\"] [--wake-app]"
            echo ""
            echo "選項:"
            echo "  --next-page \"按鍵名稱\"  指定翻頁按鍵 (預設: Left Arrow)"
            echo "  --wake-app              喚醒正在截圖的 App"
            echo "  -h, --help              顯示此說明"
            echo ""
            echo "支援的按鍵名稱:"
            ...
            echo "  $0 --wake-app                          # 喚醒 App"
            echo "  $0 --next-page \"Right Arrow\" --wake-app  # 組合使用"
            exit 0
            ;;
        *)
            echo "未知參數: $1"
...
echo "喚醒 App: $WAKE_APP"

# 執行 AppleScript 並傳遞參數
osascript ./auto-book-screenshots/interactive-screenshot.scpt "$NEXT_PAGE_KEY" "$WAKE_APP"